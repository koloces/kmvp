...
/**
 * Created by koloces on 2019/2/28
 * 银行卡相关工具
 * 根据银行卡前6位数判断银行名称和卡名称
 */
public class GetBank {
    ...
    public static String getname(String cardnumber) {
        if (StringUtil.isEmpty(cardnumber) || cardnumber.length() < 6){
            return "";
        }
...
//        return bankName[index].substring(0, 4);
        int subIndex = bankName[index].indexOf("银行") + 2;
        return bankName[index].substring(0, subIndex);
    }
